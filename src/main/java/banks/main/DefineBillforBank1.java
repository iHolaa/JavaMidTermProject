...
public class DefineBillforBank1 {
    ...
    public void ConfirmButton(){
        ...
        fm.FileReader("src\\main\\resources\\Files\\User\\AddUser\\Bank1\\LastName.txt",lastname);
        fm.FileReader("src\\main\\resources\\Files\\User\\AddUser\\Bank1\\AccountCredit.txt",credit);
        try {
            double x;
            ...
            for (int i = 0; i < name.size(); i++) {
                if (userNameTextField.getText().equals(name.get(i)) && userLastnameTextField.getText().equals(lastname.get(i)) && x >= 10 ) {
                    fm.FileWriter("src\\main\\resources\\Files\\DefineBill\\Bank1\\Name.txt",userNameTextField.getText());
                    fm.FileWriter("src\\main\\resources\\Files\\DefineBill\\Bank1\\LastName.txt",userLastnameTextField.getText());
                    fm.FileWriter("src\\main\\resources\\Files\\DefineBill\\Bank1\\credit.txt", String.valueOf(credit.get(i)));
                    fm.FileWriter("src\\main\\resources\\Files\\DefineBill\\Bank1\\BillAmount.txt",BillAmountTextField.getText());
                    confirmLabel.setText(" Bill Defined Successfully ");
                    int billCode = 100000000 + new Random().nextInt(999999999);
                    fm.FileWriter("src\\main\\resources\\Files\\DefineBill\\Bank1\\BillID.txt",String.valueOf(billCode));
